...
```{r, loading, tidy=FALSE, result="hide"}
    library(plyr)
...
```{r, daily_mean, tidy=FALSE, fig.width=12}
...
value **`r format(mean_median[1], digits=7)` steps**. The median is 
**`r format(mean_median[2], 5)` steps** which too close to the mean to be shown
in the same plot.
...
```{r, interval_mean, tidy=FALSE, fig.width=12}
...
The maximum mean occurs at **`r interval_data[2]` hours** with a value of 
**`r interval_data[3]` steps** in the interval. This is indicated on the
plot above.
...
```{r, imputing, tidy=FALSE, fig.width=12}
imputed <- raw 
...
remain at **`r format(mean_median[1], digits=7)` steps** and
**`r format(mean_median[2], 5)` steps** for the mean and median respectively.
...
```{r, label_days, tidy=FALSE, results='hide'}
...
```{r, plot_weekday_and_weekend, tidy=FALSE, fig.height=12, fig.width=12}
...
on **weekdays occurs at `r wkday_intervals[[2]]` hours** with a value of
**`r wkday_intervals[[3]]` steps**. The corresponding data during **weekends is
`r wkend_intervals[[2]]` hours** with a value of **`r wkend_intervals[[3]]` steps**